...
interface Player {
  ...
  profilePic: string;
  sports: string[];
  ratings: { sport: string; rating: number; matches: number }[];
...
const mockPlayers: Player[] = [
  {
    ...
    ratings: [
      { sport: 'Pickleball', rating: 1850, matches: 28 },
      { sport: 'Badminton', rating: 1720, matches: 15 }
    ],
    availableNow: true,
    respectScore: 92,
  ...
  },
  {
    ...
    sports: ['Badminton'],
    ratings: [
      { sport: 'Badminton', rating: 1650, matches: 22 }
    ],
    availableNow: false,
    respectScore: 88,
  ...
  },
  {
    ...
    ratings: [
      { sport: 'Table Tennis', rating: 1720, matches: 18 },
      { sport: 'Pickleball', rating: 1550, matches: 8 }
    ],
    availableNow: false,
    respectScore: 95,
    ...
    recentMatchHistory: [
      { date: '12/13', sport: 'Table Tennis', opponent: 'PaddlePro', result: 'win' },
      { date: '12/10', sport: 'Pickleball', opponent: 'NetNinja', result: 'loss' },
      { date: '12/07', sport: 'Table Tennis', opponent: 'SpinMaster', result: 'win' }
    ]
  },
  {
    id: '4',
    ...
    sports: ['Pickleball'],
    ratings: [
      { sport: 'Pickleball', rating: 1920, matches: 35 }
    ],
    availableNow: true,
    respectScore: 96,
    location: 'Downtown Courts',
    ...
    recentMatchHistory: [
      ...
      { date: '12/13', sport: 'Pickleball', opponent: 'NetMaster', result: 'win' },
      { date: '12/11', sport: 'Pickleball', opponent: 'CourtAce', result: 'win' }
    ]
  },
];
...
export default function HomeScreen() {
  ...
  const showCourtDrawer = (court: Court) => {
    ...
    if (mapRef.current) {
      ...
      // to ensure the selected marker is clearly visible above the drawer
      const verticalOffset = -0.0065; // Reduced offset for better court marker visibility
  ...
  const filteredZones = mockPlayZones.map(zone => ({
    ...
    players: zone.players.filter(player => {
      const sportMatch = player.ratings.some(rating => filters.sports.includes(rating.sport));
          ...
          <View style={styles.fullPlayerCard}>
            ...
            >
                ...
                {selectedPlayer.ratings
                  ...
                  .map((rating, index) => (
                    <View key={index} style={styles.ratingItem}>
                      <Text style={styles.sportIcon}>{getSportIcon(rating.sport)}</Text>
                      <Text style={styles.ratingText}>
                        {rating.sport} — {rating.rating}
                      </Text>
                    </View>
                  ))}
              </View>
              ...
              <View style={styles.respectSection}>
                <View style={styles.respectContainer}>
